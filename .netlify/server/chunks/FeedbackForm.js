...
const FeedbackForm = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<form method="POST" data-netlify="true" class="feedback-form flex flex-col space-y-4 text-gray-700 svelte-1olcnvn" netlify novalidate><div class="input-container svelte-1olcnvn"><div class="field-container svelte-1olcnvn"><label for="name" class="block text-sm font-medium" data-svelte-h="svelte-vsqlxf">Имя:</label> <input id="name" type="text" class="input-style svelte-1olcnvn"${add_attribute("value", name, 0)}> <span class="${escape(
    ...
    true
  ) + " svelte-1olcnvn"}">${escape(formErrors.message)}</span></div></div> <input type="hidden"${add_attribute("value", key, 0)}> <button type="submit" class="submit-button mt-10 svelte-1olcnvn" data-svelte-h="svelte-jgvucx">Отправить</button></form> ${slots.default ? slots.default({}) : ``}`;
});
export {
  FeedbackForm as F
};

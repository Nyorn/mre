...
const css$1 = {
  code: ".filter-bar.svelte-893cvm input.svelte-893cvm,.filter-bar.svelte-893cvm select.svelte-893cvm{background-color:#051518;color:white}",
  map: null
};
const FilterBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${!$isFullFilterVisible ? `<div class="filter-bar flex flex-wrap gap-2 p-2 bg-gray-700 text-white svelte-893cvm"><select class="p-2 bg-gray-800 svelte-893cvm"><option value="" data-svelte-h="svelte-1aiou29">Выберите город</option>${each(cities, (city) => {
    return `<option${add_attribute("value", city, 0)}>${escape(city)}</option>`;
  })}</select> <input type="number" placeholder="Кол-во спален" class="p-2 bg-gray-800 text-white svelte-893cvm"${add_attribute("value", bedrooms, 0)}> <div class="relative"><button class="p-2 bg-gray-800" data-svelte-h="svelte-2p6zqf">Цена</button> ${``}</div> <div class="relative"><button class="p-2 bg-gray-800" data-svelte-h="svelte-10jy4qs">Площадь</button> ${``}</div> <div class="relative"><button class="p-2 bg-gray-800" data-svelte-h="svelte-ipevkh">Этаж</button> ${``} <button data-svelte-h="svelte-l9xd7u">Применить фильтры</button> <button data-svelte-h="svelte-hlfod7">Полные фильтры</button> <button data-svelte-h="svelte-1uhjrij">Сбросить фильтры</button></div></div>` : ``}`;
});
const css = {

...
const Carousel = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  <div class="carousel svelte-1g6t9va"> <button type="button" class="carousel-button left svelte-1g6t9va" data-svelte-h="svelte-1sft7rd"><i class="fa-solid fa-arrow-left arrow-icon svelte-1g6t9va"></i></button>  <div class="carousel-container svelte-1g6t9va"${add_attribute("this", elemCarousel, 0)}>${each(images, (image, i) => {
  ...
  })}</div>  <button type="button" class="carousel-button right svelte-1g6t9va" data-svelte-h="svelte-fw07o"><i class="fa-solid fa-arrow-right arrow-icon svelte-1g6t9va"></i></button></div>  <div class="thumbnails svelte-1g6t9va">${each(images, (image, i) => {
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-1bfj62p_START -->${$$result.title = `<title>${escape(title)}</title>`, ""}<meta name="description"${add_attribute("content", description, 0)}><!-- HEAD_svelte-1bfj62p_END -->`, ""} <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold my-4">${escape(data.object.name)}</h1> <section class="flex flex-col md:flex-row justify-center items-center md:items-start md:space-x-4 px-4 py-2 svelte-9gutml">${data && data.object ? `<div class="w-full md:w-1/2">${validate_component(Carousel, "Carousel").$$render($$result, { slug: data.object.slug }, {}, {})}</div> <div class="w-full md:w-1/2 mt-4 md:mt-0"><div class="p-4 rounded-lg shadow-md"><p class="text-sm md:text-base">Город: ${escape(data.object.city)}</p> <p class="text-sm md:text-base">Цена: ${escape(data.object.price)}</p> <p class="text-sm md:text-base">Этаж: ${escape(data.object.floor)} из ${escape(data.object.maxFloor)}</p> <p class="text-sm md:text-base">Площадь: ${escape(data.object.area)}</p> <p class="text-sm md:text-base">Спален: ${escape(data.object.bedrooms)}</p> <p class="text-sm md:text-base">Лифт: ${escape(data.object.elevator ? "Есть" : "Нет")}</p> <button class="variant-ghost-tertiary p-2 bg-indigo-600 text-white rounded mt-10 mb-10" button data-svelte-h="svelte-1ti2831">Заказать осмотр</button> <div class="mt-115 map"><p class="text-base md:text-lg mt-110 svelte-9gutml"><!-- HTML_TAG_START -->${data.object.description}<!-- HTML_TAG_END --></p> <iframe width="400" height="400" title="Map" margin-top="115px" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="${"https://maps.google.com/maps?q=" + escape(data.object.location.lat, true) + "," + escape(data.object.location.lon, true) + "&z=12&output=embed"}"></iframe></div></div></div>` : `<p data-svelte-h="svelte-1ha0hdy">Информация об объекте не найдена.</p>`}</section> <hr> <section class="center svelte-9gutml" data-svelte-h="svelte-1ls1fxi"></section>`;
});
export {
  Page as default
};
